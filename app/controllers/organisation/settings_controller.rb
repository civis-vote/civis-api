class Organisation::SettingsController < ApplicationController
  ...
  before_action :set_organisation, only: %i[index create show edit update page_component hindi_page_component odia_page_component edit_hindi_summary edit_marathi_summary destroy publish edit_english_summary list_respondents destroy_respondents]
  ...
  def create
	...
	def secure_params
		params.require(:organisation).permit(:name, :logo, :official_url)
	end
