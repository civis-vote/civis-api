class Respondent < ApplicationRecord
	...
	def self.invite_respondent(consultation, organisation, params)
      	...
      	consultation_ids = consultation_response_rounds.where(response_rounds: { id: response_round_ids } ).map(&:id)
				unless consultation_ids.include? consultation.id
					...
					if consultation.published?
						url = self.respondent_invite_url(consultation, respondent.user)
						InviteRespondentJob.perform_later(consultation, respondent.user, url)
					end
				end
			end
		end
    ...
    emails.each do |email|
      ...
      if user_record
        ...
        unless respondent_record.present?
      ...
      url = self.respondent_invite_url(consultation, user_record)
	...
	def self.respondent_invite_url(consultation, user_record)
