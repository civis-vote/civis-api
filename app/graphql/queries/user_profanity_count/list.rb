module Queries
	module UserProfanityCount
	    ...
	    # description "Get a list of user profanity count mapping"
	    # argument :page,							Int,		required: false, default_value: 1
	    # argument :per_page,					Int,		required: false, default_value: 10
	    # argument :sort,							Types::Enums::UserProfanityCountSorts, 	required: false, default_value: nil
	    ...
	    type Types::Objects::UserProfanityCount::List, null: true

	    # def resolve(search_filter:, per_page:, page:, sort:, sort_direction:)
	    # 	::UserProfanityCount.search(search_filter).sort_records(sort, sort_direction).list(per_page, page)
	    # end
		def resolve()
