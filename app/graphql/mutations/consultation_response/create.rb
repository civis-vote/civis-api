  ...
  module ConsultationResponse
    class Create < Mutations::BaseMutation
      ...
      def resolve(consultation_response:)
        ...
        active_response_round_id = @consultation.response_rounds.order(:created_at).last.id
        created_consultation_response.response_round_id = active_response_round_id
        if @consultation.private_consultation?
          respondent = ::Respondent.find_by(user_id: context[:current_user].id, response_round_id: active_response_round_id)
